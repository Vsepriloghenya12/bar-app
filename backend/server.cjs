...
function verifyTelegramInitData(initData, botToken) {
  try {
    ...
    const calc = crypto.createHmac('sha256',secret).update(checkStr).digest('hex');
...
function ensureUser(id,name,roleGuess) {
  ...
  if (!u) {
    db.prepare(`INSERT INTO users (tg_user_id,name,role)
      VALUES (?,?,?)`).run(id,name||'',roleGuess);
...
app.post('/api/admin/suppliers', auth, admin, (req,res)=>{
  ...
  if (!name || name.trim().length<2) 
    return res.status(400).json({ ok:false, error:'Name short' });

  const r = db.prepare(`INSERT INTO suppliers (name,contact_note,active)
    VALUES (?,?,1)`).run(name.trim(),contact_note);
...
app.get('/api/admin/products', auth, admin, (req,res)=>{
  const rows = db.prepare(`SELECT * FROM products ORDER BY active DESC, name`).all();
...
app.post('/api/admin/products', auth, admin, (req,res)=>{
  ...
  if (!name || name.trim().length<2)
    return res.status(400).json({ ok:false, error:'Short name' });
  if (!unit)
    return res.status(400).json({ ok:false, error:'Unit required' });
...
app.patch('/api/admin/products/:id', auth, admin, (req,res)=>{
  ...
  if (newName.length<2) return res.status(400).json({ ok:false, error:'Name short' });
  if (!newUnit) return res.status(400).json({ ok:false, error:'Unit required' });
...
app.post('/api/admin/products/:id/suppliers', auth, admin, (req,res)=>{
  ...
  if (!Number.isFinite(sid))
    return res.status(400).json({ ok:false, error:'Bad supplier_id' });
  ...
  /* передвигаем самого первого */
  const current = getSuppliersForProduct(pid);
  if (current.length === 1) {
    db.prepare(`
...
app.delete('/api/admin/products/:id/suppliers/:sid', auth, admin, (req,res)=>{
  ...
  /* пересортировать оставшихся */
  const rows = getSuppliersForProduct(pid);
  rows.forEach((x,i)=>{
    db.prepare(`
      UPDATE product_suppliers SET sort_order=?
      WHERE product_id=? AND supplier_id=?
    `).run(i+1,pid,x.supplier_id);
  });
...
app.post('/api/admin/products/:id/set-primary/:sid', auth, admin, (req,res)=>{
  ...
  rows.forEach(x=>{
    const newOrder = (x.supplier_id===sid ? 1 : x.sort_order+1);
    db.prepare(`
      UPDATE product_suppliers SET sort_order=?
      WHERE product_id=? AND supplier_id=?
    `).run(newOrder,pid,x.supplier_id);
  });
...
app.post('/api/requisitions', auth, (req,res)=>{
  ...
  const trx = db.transaction(()=>{
    ...
    const insOrder = db.prepare(`
      INSERT INTO orders (requisition_id,supplier_id)
      VALUES (?,?)
    `);
    ...
    for(const it of items){
      ...
      if (!primary)
        throw new Error(`Товар ID ${pid} не имеет поставщиков`);
...
app.get('/api/admin/requisitions/:id', auth, admin, (req,res)=>{
  ...
  const itemsStmt = db.prepare(`
    SELECT i.product_id, p.name AS product_name, p.unit, i.qty_requested
    FROM order_items i
